...
properties:{
        ...
        mounted(){this.extractIdsFromUrl();
},
      layout: 'auth-wl',
        methods: {
extractIdsFromUrl() {
     const pathSegments = window.location.pathname.split('/');
      this.landlord_id = pathSegments[2]; // Assuming 'landlord_id' is at index 3
      this.id_reg = pathSegments[3];
      
      console.log('Landlord ID:', this.landlord_id);
      console.log('User ID:', this.id_reg);
    },            
 ...
 if(response.status=='ok'){
	...
	setTimeout(()=>{
window.location.href='https://portal.watenantbureau.com/message';
}, 2000);                     
}else{
            const errorMessage = response.message ? response.message.sqlMessage : 'An unknown error occurred'; 
                    this.$toast.error(JSON.stringify(errorMessage),{duration:2000,fitToScreen:true});
                 }
}catch(error){
...
<div class="form-tabs-title">
   <input type="text" name="landlord_id" id="landlord_id" v-model="landlord_id" placeholder="Landlord ID" />
    <input type="text" name="id_reg" id="id_reg" v-model="id_reg" placeholder="User ID" />
