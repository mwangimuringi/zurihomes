        ...
        methods: {
            ...
            async asyncData() {
                let response = await this.$axios.$post("/tenants/rentalapp-list/dt/", {
                    landlord_id: this.$auth.user.user_login_id,
                ...
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            ...
                            <!-- <nuxt-link to="/rental-app/1-index" class="btn btn-danger mb-2"><i class="mdi mdi-plus-circle mr-1"></i> Add Rental Application</nuxt-link> -->
			   <nuxt-link to ="/send-link/sendLink" class="btn btn-success mb-2"><i class="mdi mdi-plus-circle mr-1"></i>Send Link</nuxt-link>                    
    </div>
                        <div class="col-sm-6">
                    ...
                    <div class="table-responsive mb-0">
                        <b-table table-class="table table-centered w-100" thead-tr-class="bg-light" :items="asyncData" :fields="fields" responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered">
                            ...
                            <template v-slot:cell(inputDisclosure)="data">
                                 {{ _app_format_date_time(data.item.inputDisclosure) }}
                            </template>
                            <template v-slot:cell(Sex)="data">
                                 {{ _app_onGetSexName(data.item.Sex) }}
                            </template>
                            ...
                            <template v-slot:cell(status_verified)="data">
                                <span class="badge p-1 badge-soft-success" :class="{'badge-soft-danger': data.item.status_verified === 0}">{{ _app_onGetStatusRA(data.item.status_verified) }}</span>
                            </template>
